...
const MyWalletTab = () => {
  ...
  const { userDetails, accessToken } = useSelector((state) => state.auth);
  const { isLoggedIn } = useCustomerLoggedIn();
  ...
  const handleFetchWalletAmount = async () => {
    try {
      ...
      setWalletAmount(response?.data?.txns?.[0]?.wallet_balance || 0);
    } catch (error) {
  ...
  return (
    <>
      <Helmet>
        <title>Manage Wallet Amount - Secure Wallet | Cureka</title>
        <meta
          name="description"
          content="Safely manage your saved cards and payment methods in your Wallet at Cureka."
        />
        <link rel="canonical" href={window.location.href} />
        <meta property="og:url" content={window.location.href} />
        <meta property="og:type" content="website" />
      ...
      <div
        ...
        role="tabpanel"
      >
        <h1 className="mb-4 fw-bold">My Wallet</h1>

        {isLoggedIn && (
          <div className="wallet-section">
            <div className="d-flex justify-content-between align-items-center border rounded p-4 shadow-sm mb-4 bg-white">
              <div>
                <h5 className="text-muted mb-1">Current Balance</h5>
                <h3 className="text-primary fw-bold">₹{walletAmount}</h3>
              </div>
              <Button
                variant="primary"
                onClick={handleShow}
                style={{ backgroundColor: "#e75204", color: "#fff" }}
              >
                Add Money to Wallet
              </Button>
            </div>
            ...
            {transactionData.length > 0 && (
              <>
                <h4 className="mt-5 mb-3 fw-semibold">Recent Transactions</h4>
                {transactionData.map((txn) => (
                  ...
                  >
                    ...
                    <small className="text-muted">
                      {new Date(txn.txn_date).toLocaleString("en-US", {
                        year: "numeric",
                        month: "short",
                        day: "numeric",
                        hour: "numeric",
