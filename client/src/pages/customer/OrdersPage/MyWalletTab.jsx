...
const MyWalletTab = () => {
  ...
  const handleFetchTransactions = async () => {
    ...
    const response = await axios.get(`${env.REACT_SERVER_BASE_URL}/user/account/fetchtxns`, {
      headers: {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`${env.REACT_SERVER_BASE_URL}/api/v1/walletpayment`, formData);
  ...
  return (
    <>
      <Helmet>
        <title>Manage Wallet Amount - Secure Wallet & Saved Cards | Cureka</title>
        <meta name="description" content="Safely manage your saved cards and payment methods in your Wallet at Cureka. Add, edit, or remove payment options for quick and secure checkout every time. Update your wallet now!." />
        <link rel="canonical" href={window.location.href} />
        <meta property="og:url" content={window.location.href} />
        <meta property="og:type" content="website" />
        <meta property="og:title" content="Cureka" />
        <meta property="og:description" content="Safely manage your saved cards and payment methods in your Wallet at Cureka. Add, edit, or remove payment options for quick and secure checkout every time. Update your wallet now!." />
            ...
            <div className="col-lg-12">
              <div className="d-flex justify-content-between wallet-border">
                <div>
                  ...
                  <p className="rupees">₹ {getWalletAmount && getWalletAmount[0] && getWalletAmount[0].wallet_balance}</p>
                </div>
                    ...
                    <Modal show={show} onHide={handleClose} id="rateproductModal">
                      ...
                      <Modal.Body>
                        <Form onSubmit={handleSubmit}>
                          ...
                          <div>
                            <Button variant="primary" type="submit" style={{ backgroundColor: "#004a98" }}>
                              Submit
                            </Button>
                          </div>
                        </Form>
              ...
              {getTransactionData && getTransactionData.length > 0 &&
                (
                  <>
                    {getTransactionData && getTransactionData.map((item) => (
                      <div key={item.id}>
                        <div className="d-flex justify-content-between">
                          <h2 className="money">{item.transaction_type == "CREDIT" ? "Amount Credit" : "Amount Debit"}</h2>
                          <p
                            className="credit-price"
                            style={{ color: item.transaction_type !== "CREDIT" ? 'red' : 'green' }}
                          >
