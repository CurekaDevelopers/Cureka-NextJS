...
const MyWishlistTab = () => {
  ...
  return (
    <>
      <Helmet>
        <title>Manage Wishlist - Save Favorite Products & Shop Later | Cureka</title>
        <meta name="description" content="Organize and manage your wishlist on [Your Store Name]. Save your favorite products, track availability, and shop later with ease. Create a personalized shopping experience today." />
        <link rel="canonical" href={window.location.href} />
        <meta property="og:url" content={window.location.href} />
        <meta property="og:type" content="website" />
        <meta property="og:title" content="Cureka" />
        <meta property="og:description" content="Organize and manage your wishlist on [Your Store Name]. Save your favorite products, track availability, and shop later with ease. Create a personalized shopping experience today" />
        ...
        <div className="row">
          <div className="col-lg-12 px-0">
            {wishlistProducts?.length ? (
              <div className="order-display rounded-3">
                {wishlistProducts.map((item, index) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className={`order-card rounded-0 ${index === wishlistProducts?.length - 1 ? "border-bottom-0" : ""
                        }`}
                    >
                        ...
                        <a href={generateUrl(item)} target="_blank">
                          <div className="d-lg-flex d-flex-column">
                            ...
                            <div className="product-details">
                              <h2 className="heading">{item.vendor_article_name}</h2>

                              <p className="discount">₹ {Number(item.mrp).toFixed(1)}</p>

                              <p className="price">₹ {Number(item.final_price).toFixed(1)}</p>
                            </div>
                          </div>
                        </a>
              ...
              <p className="price" style={{ textAlign: "center" }}>"You don’t have an wishlist."</p>
            )}
          </div>
        </div>
      </div >
