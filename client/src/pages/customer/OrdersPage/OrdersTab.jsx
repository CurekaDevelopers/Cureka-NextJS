...
import { addRateAndReview, fetchCustomerOrders, fetchProductBySlug } from "../../../redux/action/index";
...
const OrdersTab = () => {
  ...
  const handleDownloadInvoice = async () => {
    try {
      const response = await axios.get(`${env.REACT_SERVER_BASE_URL}/api/v1/checkout/getUniInvoice/${order.order_id}`, {
        responseType: 'blob'
      });
  ...
  const generatePDF = (orderData) => {
    ...
    // Company logo
    const logo = new Image();
    logo.src = "https://www.cureka.com/wp-content/uploads/2020/11/logo-dark-3.png";
    doc.addImage(logo, "PNG", 350, 120);
    ...
    doc.text(`${new Date(orderData[0].created_at).toLocaleDateString("en-IN", {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric'
    }).replace(/\//g, '/')}`, 475, 240);
    ...
    doc.text(`${orderData[0].shipping_address.mobile},${orderData[0].shipping_address.email}`, 50, 260);
    doc.text(`${orderData[0].shipping_address.address}`, 50, 280);
    doc.text(`${orderData[0].shipping_address.landmark},${orderData[0].shipping_address.city},${orderData[0].shipping_address.state},${orderData[0].shipping_address.pincode}`, 50, 300);
    ...
    let onePerDiscountApplied;
    const subtotal = orderData[0].subtotal ? orderData[0].subtotal : 0.00;
    const discount = orderData[0].discount !== null ? orderData[0].discount : 0.00;
    const shippingCharge = Number(orderData[0].shippingCharge).toFixed(2) !== undefined ? Number(orderData[0].shippingCharge).toFixed(2) : 0.00;
    const transactionType = orderData[0].is_cod == 1 ? "COD" : orderData[0].is_wallet_option == 1 ? "Wallet Transaction" : "Online Transaction";
    let walletMoneyUsed = Number(orderData[0].walletMoneyUsed).toFixed(2);
    let gift_wrapping = orderData[0].gift_wrapping;

    const totalItemSum = orderData.reduce((acc, product) => acc + (product.final_price * product.quantity), 0);
    if (totalItemSum > 599 && transactionType == "Online Transaction") {
      onePerDiscountApplied = totalItemSum * 0.01;
    }
    ...
    if (onePerDiscountApplied > 0) {
      ...
      doc.text(`-${onePerDiscountApplied}`, 470, doc.autoTable.previous.finalY + 80);
    } else {
      doc.text("1% Dis above 599:", 370, doc.autoTable.previous.finalY + 80);
  ...
  const handleSubmitCancelOrder = async () => {
    if (cancellationReason) {
      ...
      try {
        const response = await axios.post(`${env.REACT_SERVER_BASE_URL}/api/v1/checkout/cancelOrder`,
          formData, {
          headers: {
  ...
  const handleChange = (e) => {
    ...
    if (!isNaN(orderId)) {
      ...
      const results = listApproved.filter(order => order.order_id === orderId);
      if (results.length > 0) {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Track your orders, view purchase history, and manage returns or exchanges on Cureka. Stay updated on your order status and enjoy a seamless shopping experience with easy order management." />
        <link rel="canonical" href={window.location.href} />
        <meta property="og:url" content={window.location.href} />
        <meta property="og:type" content="website" />
        <meta property="og:title" content="Cureka" />
        <meta property="og:description" content="Track your orders, view purchase history, and manage returns or exchanges on Cureka. Stay updated on your order status and enjoy a seamless shopping experience with easy order management." />
      ...
      >
            ...
            {showFirstDiv ?
              <>
                ...
                <input
                  ...
                  className="mb-3"
                  style={{
                    padding: " 0.375rem 0.75rem", borderRadius: "0.25rem", border: " 1px solid #ced4da"
                  }}
                />
                {listApproved && listApproved.length > 0 ? (
                  <>
                    {listApproved && listApproved.length > 0 && listApproved?.map((order, i) => (
                      order?.products.map((product, j) => (
                        <div className="order-display">
                          <div className="order-card border-bottom-0" key={i}>
                            <div className="d-lg-flex d-flex-column justify-content-between" key={j}>
                              <div className="d-lg-flex d-flex-column">
                                <div onClick={(e) => toggleDiv(product, order)} className="cursorPointer card align-self-center">
                                  <img
                                    className="img-fluid mx-auto"
                                    src={product.product_image_array[0]?.image} // order1 is default image you can change as per your requirement
                                    width="73px"
                                    height="80px"
                                    alt="order1"
                                  />
                                ...
                                <div className="product-details">
                                  <h2 onClick={(e) => toggleDiv(product, order)} className="cursorPointer heading">
                              ...
                              <div id="dot">
                                <p className='align-center'>Order Id: {order.order_id}</p>
                              ...
                              Re Order
                                  ...
                                  <button
                                    ...
                                    onClick={(e) => handleShow(product, order.user_id)}>
                                    Rate Product
                                  </button>
                                </div>
                              </div>
                  ...
                  <>
                    <p className="price" style={{ textAlign: "center" }}>"You don’t have an order. Please try placing a new order."</p>
                  </>
                )}

              </> : <>
                {order && <div className="row" id="order-id">
                  <button onClick={toggleDiv} className="order-heading heading-add">
                    {`Order Id : #${order.order_id}`}
                  </button>
                  <div className="col-lg-12 order-details p-0">
                    {order && order.products.map((product, k) =>
                      <div key={k}>
                          ...
                          <div className="d-lg-flex d-flex-column justify-content-between">
                            <div className="d-lg-flex d-flex-column">
                              <div className="card align-self-center">
                                <img
                                  className="img-fluid mx-auto"
                                  src={product.product_image_array[0]?.image}
                                  width="73px"
                                  height="80px"
                                  alt="order1"
                                />
                              ...
                              <div className="product-details">
                                ...
                                <p className="price">Quantity: {product.quantity}
                                </p>
                            ...
                            {k === 0 && <div id="dot">
                              <div className="d-flex justify-content-start">
                                <Button type='btn' className="text-decoration-none rate-btn mr-0" onClick={handleDownloadInvoice}>Download Invoice</Button>
                              </div>
                              <p style={{ color: "black", fontWeight: "600" }}> Order Created: {createdOrderDate(order.created_at)}  </p>
                              <p style={{ marginTop: "5x", marginBottom: "4px", color: "black", fontWeight: "600" }}> Order Status: {order && order.order_placed_status}</p>
                              {order.order_placed_status == "Cancelled" ? (
                                <>
                                </>
                              ) : (
                                <>
                                  <div className="d-flex justify-content-start">
                                    <Button type='btn' className="text-decoration-none rate-btn mt-1" onClick={(e) => handleCancelOrder()}>Cancel Order</Button>
                                  </div>
                                </>
                              )
                              }
                        ...
                        {k === 0 && order?.trackInfo?.scans?.length > 0 &&
                          <div className='ml-4 mb-3 mt-3'>
                            <a type='btn' className="text-decoration-none mt-1"
                              style={{ color: "#e75204", cursor: "pointer" }}
                              onClick={handleTrackingView}>View Tracking</a>
                          </div>
                        }

                        {k === 0 &&
                          <div className='mb-4'>
                            {order?.trackInfo?.scans?.map((item, index) => (
                              trackView && (
                                <div className="d-flex" key={index} id="tracking">
                                  ...
                                  <div>
                                    <p className="order-confirm">{item['sr-status-label']}</p>
                                    <p className="confirm-date">{trackerFormatDate(item.date)}</p>
                                  </div>
                                </div>
                              )
                            ))}
                        ...
                        <div className='row'>
                          <div className='col-lg-4'>
                            <div className="delivery-address">
                              <h2 className="heading mt-3">Delivery Address</h2>

                              <p className="location-one">{order && order.shipping_address && order.shipping_address.name}</p>
                              <p className="location-one">{order && order.shipping_address && order.shipping_address.email}</p>
                          ...
                          Colony, Madurai, Tamil Nadu 625007 */}
                                {order && order.shipping_address && order.shipping_address.address_type} {":"}
                                {order && order.shipping_address && order.shipping_address.address}, {order && order.shipping_address && order.shipping_address.landmark},
                                {" "} {order && order.shipping_address && order.shipping_address.shipping_city},{" "} {order && order.shipping_address && order.shipping_address.shipping_state} {"-"} {order && order.shipping_address && order.shipping_address.pincode}
                              </p>

                              <p className="location-one">Phone number</p>

                              <p className="address-one">{order && order.shipping_address && order.shipping_address.mobile}</p>
                            </div>
                          </div>
                          <div className='col-lg-4'>
                          ...
                          <div className='col-lg-4'>
                            ...
                            {k === 0 &&
                              <div className='mt-3'>
                                <p className="price">Payment Menthod:
                                ...
                                {order && order.products.length > 0 &&
                                  <p className="price">Items Total:<span style={{ marginLeft: "55px" }}>₹ {order && order.products.reduce(
                                ...
                                {order && order.discount > 0 &&
                                  <p className="price">Discount:<span style={{ marginLeft: "66px" }}>₹ {order.discount}</span>
                                  </p>
                                }
                              ...
                              0
                                ...
                                {order && order.products.reduce(
                                  (acc, product) => acc + (product.final_price * product.quantity),
                                  0
                                ) > 599 && order.is_cod != true && order.is_wallet_option != true &&
                                  <p className="price">1% Dis on above 599:
                                    <span>₹ {order && order.products.reduce(
                                      (acc, product) => acc + (product.final_price * product.quantity),
                                      0
                                    ) * 0.01}</span>
                                  </p>
                                ...
                                {order && order.shippingCharge > 0 &&
                                  <p className="price">Shipping Charges:
                                    <span className='ml-1'>₹ {order.shippingCharge} </span>
                                  </p>
                                }
                                {order && order.gift_wrapping == true &&
                                  <p className="price">Gift Wrapping:
                                    <span style={{ marginLeft: "32px" }}>₹ 40</span>
                                  </p>
                                }
                                {order && order.walletMoneyUsed > 0 &&
                                  <p className="price">Wallet Money:
                                    <span style={{ marginLeft: "32px" }}> ₹ {order.walletMoneyUsed} </span>
                                  </p>
                                }

                                <p className="price">Total Amount:
                                  <span style={{ marginLeft: "32px" }}>₹ {order.subtotal} </span>
                                </p>
                              </div>
                            }
                          </div>
                ...
                <div className="d-lg-flex d-flex-column card-border">
                  ...
                  <div className="product-details">
                    ...
                    <p className="productrate align-self-center">4.8</p> */}
                      <Rating name="simple-controlled" readOnly value={ratingReview?.ratingCount?.average} />
                      <p className="productrate align-self-center">{ratingReview?.ratingCount?.average}</p>

                      <p className="reviewrate align-self-center">
                        {ratingReview?.ratingCount?.totalReviews} Ratings &amp;  {ratingReview?.ratingCount?.totalReviews} Reviews
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="review-products">
                  ...
                  <div className="form-group">
                    ...
                    <input
                      ...
                      onChange={(e) => setReviewTitleDescription(e.target.value)}
                    />
                  </div>

                  <div className="submit-btn">
                    <Button onClick={handleRateAndReview} className="text-decoration-none rate-btn">
                      Submit
                    </Button>
                  </div>
                </div>
        ...
        <Modal show={orderCancelClose} onHide={handleOrderCancelClose} id="rateproductModal">
          <Modal.Header closeButton className="mx-0"></Modal.Header>
          <div className="modal-dialog">
            <div className="modal-content border-0">
              <div className="modal-body d-flex-column justify-content-center">
                ...
                <div className="review-products">
                  ...
                  <div className="submit-btn">
                    <Button onClick={handleSubmitCancelOrder} className="text-decoration-none rate-btn">
                      Submit
                    </Button>
                  </div>
                </div>
