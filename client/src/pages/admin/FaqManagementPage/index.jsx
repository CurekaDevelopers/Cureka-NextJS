...
const FaqManagementPage = () => {
    ...
    if (deleteFaq?.id && faqs?.length) {
      dispatch(
        deleteFaqs(deleteFaq.id, () => {
          const updatedFaqsList = faqs.filter((item) => item.id !== deleteFaq.id);
  ...
  const onEdit = useCallback((id) => {
    navigate(pagePaths.adminCreateFaqEdit.replace(":id", id));
  }, [navigate]);

  const closeDeleteConfirmModal = () => {
    setDeleteFaq(null);
  };
  const rolesPermission = userRoles.filter(item => item.name == "FAQ Management")
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      accessor: "created_at",
      Cell: ({ cell }) => {
        return <span className={styles.titleColumn}>{moment(cell.row.original.created_at).format('ll')}</span>;
      },
    },
    {
      Header: "Status",
          ...
          <Card className={styles.card}>
            ...
            <div className="d-flex">
              ...
              <div className="ml-auto">
                {isAdminStatus == 1 ? (
                  <Link to={pagePaths.adminCreateFaq}>
            ...
            {mappedFaqs?.length ? (
              ...
              <p className={styles.noRecordFoundMessage}>No faqs found, please add new faq.</p>
            )}
          </Card>
        </div>
      </div>
