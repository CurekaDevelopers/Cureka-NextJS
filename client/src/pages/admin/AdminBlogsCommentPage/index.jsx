...
import { blogCommentAction, fetchBlogsListUserComment } from "../../../redux/action";
...
const AdminBlogsPage = () => {
    ...
    const { id } = useParams();
    const [blogToDelete, setBlogToDelete] = useState({ isShow: null, id: null, type: null });
    ...
    const onDeleteConfirmed = () => {
        if (blogToDelete?.id && blogsComments?.results.length) {
            dispatch(
                blogCommentAction({ id: blogToDelete.id, action: blogToDelete?.type }, () => {
                    const updatedBlogsList = blogsComments?.results?.filter((item) => item.id !== blogToDelete.id);
                    dispatch(setBlogs(updatedBlogsList || []));
                    // setBlogToDelete({ isShow: false, id: null, type: null });

                    setBlogToDelete(null)
                    navigate(pagePaths.adminBlogs);
    ...
    const onEdit = useCallback(
        ...
        (id, replay = 'edit') => {
            ...
            if (replay === 'replay') {
                navigate(pagePaths.adminCreateBlogsComment.replace(":id", id));
                return
            }
            navigate(pagePaths.adminCreateBlogsEdit.replace(":id", id));
        },
        [navigate],
    );
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                accessor: "comment",
                Cell: ({ cell }) => {
                    return <span className={styles.titleColumn}>{cell.row.original.comment ? cell.row.original.comment : "N/A"}</span>;
                },
            },
            {
                Header: "User Name",
                accessor: "user_name",
                Cell: ({ cell }) => {
                    return <span className={styles.titleColumn}>{cell.row.original.user_name ? cell.row.original.user_name : "N/A"}</span>;
                },
            },
            {
                Header: "User Email",
                accessor: "user_email",
                Cell: ({ cell }) => {
                    return <span className={styles.titleColumn}>{cell.row.original.user_email ? cell.row.original.user_email : "N/A"}</span>;
                },
            },
            {
                Header: "Comment Date",
            ...
            // },
            {
                ...
                Cell: ({ cell }) => {
                    return (
                        <>
                            <AcceptButton onClick={() => onDeleteButtonClicked(cell.row.original.id, 'accept')} />
                            <RejectButton onClick={() => onDeleteButtonClicked(cell.row.original.id, 'reject')} />
                        </>
                    );
                },
            },
    ...
    return (
        <>
            <ConfirmationModel
                ...
                label={"Message"}
                message={`Are you sure you want to  ${blogToDelete?.type === 'reject' ? "reject" : "accept"} the user message, Id: ${blogToDelete?.id}`}
                ...
                <Button className={styles.addButton}>
                  <MdAdd /> Add Blog
                        ...
                        {mappedBlogs?.length ? (
                            ...
                            <p className={styles.noRecordFoundMessage}>No blog comments found.</p>
                        )}
                    </Card>
                </div>
            </div>
