...
import { fetchProductsOptions, getAddressOnPincode } from "../../../redux/action";
...
const AdminCreateOrderPage = () => {
    ...
    const toggleOption = (newSelected) => {
        ...
        const updatedSelectedOptions = new Set(newSelected.map(item => item.value));
    ...
    const handleProductChange = (option) => {
        ...
        if (selectedProducts.has(option.value)) {
            ...
            newSelected.splice(newSelected.findIndex(item => item.value === option.value), 1);
        } else {
            // Add the new option
    ...
    const handleProductQuantity = (id, newQuantity) => {
        ...
        const shippingAmtCharge = shippingChargesData && shippingChargesData.find(r => totalAmount >= r.min && totalAmount <= (r.max ?? Infinity));
    ...
    const removeProductItem = (id) => {
        const updatedItems = priceData && priceData.filter(item => item.id !== id);
    ...
    const handlePaymentMethod = (event) => {
        ...
        const shippingAmtCharge = shippingChargesData && shippingChargesData.find(r => totalAmt >= r.min && totalAmt <= (r.max ?? Infinity));
        setShippingCharge(shippingAmtCharge);
    }
    ...
    const handleApplyCoupon = async () => {
        if (selectedProducts.size > 0) {
            if (coupon) {
                ...
                try {
                    const response = await axios.post(`${env.REACT_SERVER_BASE_URL}/admin_apply_coupon`, postData);
    ...
    const handleMobileNumberChange = async (event) => {
        ...
        if (/^\d{0,10}$/.test(value)) { // Regex to match exactly 10 digits
            setMobileNumber(value);
            if (value.length === 10) {
                try {
                    const trimmedNumberInput = value.trim(); // Trim whitespace
                    const response = await axios.get(`${env.REACT_SERVER_BASE_URL}/user/account/userDetailsAddress/${trimmedNumberInput}`);
    ...
    const onChangeUserDetails = (event) => {
        const data = userDetails && userDetails.address && userDetails.address.length > 0 && userDetails.address.find(item => item.id == event.target.value);
    ...
    const handleBillingAddressChange = async (event) => {
        ...
        if (name === 'pincode') { // Assuming pincode should be 6 digits
            try {
    ...
    const handleSubmit = async (event) => {
        ...
        if (selectedProducts.size > 0 && AddressType && payMethod) {
            ...
            if (couponDiscount) {
                if (isBillingAndShippingSame) {
                    formData = {
                        ...
                        orderData: {
                            subtotal: isGiftWrapping ? Math.round(Number(couponDiscount && couponDiscount.final_amount)) + (shippingCharge && shippingCharge.charge) + Number(40) : Math.round(Number(couponDiscount && couponDiscount.final_amount)) + (shippingCharge && shippingCharge.charge),
                            discount: Number(couponDiscount && couponDiscount.discount) ? Number(couponDiscount && couponDiscount.discount) : 0.00,
                            coupon_code: coupon,
                ...
                if (isBillingAndShippingSame) {
                    formData = {
                        ...
                        orderData: {
                            subtotal: isGiftWrapping ? Math.round(Number(totalAmt)) + (shippingCharge && shippingCharge.charge) + Number(40) : Math.round(Number(totalAmt)) + (shippingCharge && shippingCharge.charge),
                            discount: Number(couponDiscount && couponDiscount.discount) ? Number(couponDiscount && couponDiscount.discount) : 0.00,
                            coupon_code: coupon,
                            gift_wrapping: isGiftWrapping,
                            transaction_id: transactionId ? transactionId : null,
            ...
            // return
            try {
                const response = await axios.post(`${env.REACT_SERVER_BASE_URL}/api/v1/checkout/admin-place-order`, formData);
            ...
            <div>
                <Card className={styles.card}>
                    ...
                    <Form onSubmit={handleSubmit} className={styles.formItems}>
                        <Form.Group>
                            <Form.Label>User Full Name <span className="text-danger">*</span></Form.Label>
                            <Form.Control
                                type="text"
                                value={firstName}
                        ...
                        <Form.Group>
                            <Form.Label>User Address<span className="text-danger">*</span></Form.Label>
                            <Form.Select
                                name="addtype"
                            ...
                            >
                                ...
                                {userDetails && userDetails.address && userDetails.address.length > 0 && userDetails.address.map((option) => (
                        ...
                        {payMethod == "onlineTransaction" &&
                            <Form.Group>
                                <Form.Label>Enter Transaction ID <span className="text-danger">*</span></Form.Label>
                                <Form.Control
                                    type="text"
                                    name="state"
                                    value={transactionId}
                        ...
                        {priceData.length > 0 && (
                            <>
                                                    ...
                                                    <tr key={productName.id}>
                                                        ...
                                                        <td>
                                                            <select
                                                                ...
                                                                onChange={(e) => handleProductQuantity(productName.id, parseInt(e.target.value))}
                              ...
                              {productName.quantity?productName.quantity*productName.final_price:productName.final_price}
                                                        ...
                                                        <td>{calculateSubtotal(productName.final_price, productName.quantity)}</td>

                                                        <td>
                                                            <button
                                                                className="btn btn-danger btn-sm"
                                                                onClick={() => removeProductItem(productName.id)}
                                                            >
                        ...
                        <Form.Group>
                            <Form.Label style={{ color: "#ff9800", marginright: "20" }}>Shipping charges:  </Form.Label>
                            ₹ {shippingCharge && shippingCharge.charge}
                        </Form.Group>
                        <Form.Group>
                            <Form.Label style={{ color: "#ff9800", marginright: "20" }}>Sub Total:  </Form.Label>
                            ₹{Number(totalAmt)}
                        </Form.Group>
                        {!isGiftWrapping && !couponDiscount &&

                            <Form.Group>
                                <Form.Label style={{ color: "#ff9800", marginright: "20" }}>Amount to be paid:  </Form.Label>
                        ...
                        {isGiftWrapping && !couponDiscount && (
                            <>
                                <Form.Group>
                                    <Form.Label style={{ color: "#ff9800", marginright: "20" }}>Gift wrapping charges:  </Form.Label>
                                    ₹ 40
                                </Form.Group>
                                <Form.Group>
                                    <Form.Label style={{ color: "#ff9800", marginright: "20" }}>Amount to be paid:  </Form.Label>
                                    ₹{Math.round(Number(totalAmt) + (shippingCharge && shippingCharge.charge)) + Number(40)}
                                </Form.Group>
                            </>
                        )
                        }
                        {!coupon == " " && couponDiscount && (
                            <>
                                ...
                                <Form.Group>
                                    <Form.Label style={{ color: "#ff9800", marginright: "20" }}> Discount ({couponDiscount &&
                                        couponDiscount.discount_percentage}% Off) </Form.Label>
                                ...
                                <Form.Group>
                                    ...
                                    {isGiftWrapping ? Math.round(Number(couponDiscount && couponDiscount.final_amount)) + (shippingCharge && shippingCharge.charge) + Number(40) : Math.round(Number(couponDiscount && couponDiscount.final_amount)) + (shippingCharge && shippingCharge.charge)}
                                </Form.Group>
                            </>
                        )}
                        <Form.Label> Fields marked with <span className="text-danger">*</span> are mandatory</Form.Label>
                        <Button
                            type="submit"
                            className={styles.submitButton}
                            variant="primary"
