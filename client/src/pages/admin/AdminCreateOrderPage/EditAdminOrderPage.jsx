...
const EditAdminOrderPage = () => {
    ...
    const fetchEditData = async () => {
        // setLoading(true);
        try {
            const response = await axios.get(`${env.REACT_SERVER_BASE_URL}/api/v1/checkout/getAdminOrderDetails/${id}`);
            ...
            setBaddress(result && result[0] && result[0].billing_address && result[0].billing_address.address)
            setBlandmark(result && result[0] && result[0].billing_address && result[0].billing_address.landmark)
            setBpincode(result && result[0] && result[0].billing_address && result[0].billing_address.pincode)
            setBcity(result && result[0] && result[0].billing_address && result[0].billing_address.city)
            setBstate(result && result[0] && result[0].billing_address && result[0].billing_address.state)
            setBAddressType(result && result[0] && result[0].billing_address && result[0].billing_address.address_type)
            setBMobile(result && result[0] && result[0].billing_address && result[0].billing_address.mobile)
            setBEmail(result && result[0] && result[0].billing_address && result[0].billing_address.email)
            setBAddressId(result && result[0] && result[0].billing_address_id)


            //set shipping address
            setSaddress(result && result[0] && result[0].shipping_address && result[0].shipping_address.address)
            setSlandmark(result && result[0] && result[0].shipping_address && result[0].shipping_address.landmark)
            setSpincode(result && result[0] && result[0].shipping_address && result[0].shipping_address.pincode)
            setScity(result && result[0] && result[0].shipping_address && result[0].shipping_address.city)
            setSstate(result && result[0] && result[0].shipping_address && result[0].shipping_address.state)
            setAddressType(result && result[0] && result[0].shipping_address && result[0].shipping_address.address_type)
            setSMobile(result && result[0] && result[0].shipping_address && result[0].shipping_address.mobile)
            setSEmail(result && result[0] && result[0].shipping_address && result[0].shipping_address.email)
    ...
    const handleBillingAddressSubmit = async (event) => {
        ...
        formData = {
            ...
            user_id: editData && editData[0] && editData[0].billing_address && editData[0].billing_address.user_id,
            billing_address: {
                // id: bAddressId,
                user_id: editData && editData[0] && editData[0].billing_address && editData[0].billing_address.user_id,
                name: editData && editData[0] && editData[0].billing_address && editData[0].billing_address.name,
                email: bemail,
                mobile: bmobile,
                address: baddress,
                landmark: blandmark,
            ...
            shipping_address: {
                id: sAddressId,
                user_id: editData && editData[0] && editData[0].shipping_address && editData[0].shipping_address.user_id,
                name: editData && editData[0] && editData[0].shipping_address && editData[0].shipping_address.name,
                email: semail,
                mobile: smobile,
                address: saddress,
                landmark: slandmark,
        ...
        // return;
        try {
            const response = await axios.put(`${env.REACT_SERVER_BASE_URL}/api/v1/checkout/updateAdminOrderDetails/${id}`, formData);
            let data = response.data;
            if (data) {
                setBillingModalPopupShow(false);
                toast.success("Billing Address Updated Successfully");
                fetchEditData();
                // navigate(pagePaths.adminOrders);

            }
            navigate(pagePaths.adminEditOrder);
    ...
    const handleInvoiceDownload = async () => {
        try {
            const response = await axios.get(`${env.REACT_SERVER_BASE_URL}/api/v1/checkout/getUniInvoice/${id}`, {
                responseType: 'blob'
            });
            ...
            if (error.response) {
                if (error.response.status === 500) {
                    toast.error('Server error (500): Unable to process your request. Please try again later.');
                } else if (error.response.status === 404) {
                    toast.error('Invoice not found (404): Please check the order ID and try again.');
                } else {
    ...
    const handleOrderStatusSubmit = async () => {
        ...
        try {
            const response = await axios.put(`${env.REACT_SERVER_BASE_URL}/api/v1/checkout/updateStatus/${id}`, formData);
            if (response && response.data) {
    ...
    const trackerFormatDate = (dateString) => {
        ...
        const formattedDate = date.toLocaleDateString('en-US', options).replace(/(\d+)(th|st|nd|rd)/, '$1') + `'`;
        const timeOptions = { hour: 'numeric', minute: 'numeric', hour12: true };
        const formattedTime = date.toLocaleTimeString('en-US', timeOptions).replace(':00', '');
                ...
                <Card className={styles.card}>
                    ...
                    <div className="row">
                        <Card className={`col-lg-8 ${styles.card}`}>
                            <div className="row">
                                <h4>Order {editData && editData[0] && editData[0].order_id} details</h4>
                                <p>Payment via {editData && editData[0] && editData[0].is_cod == 1
                                    ? "Cash on Delivery" : editData[0] && editData[0].is_wallet_option == 1 ? "Wallet Transaction" : "Online Transaction"}
                                </p>
                            </div>
                            <div className="row">
                                <div className="col-4 mt-3">
                                    ...
                                    <div>
                                        <Button className={styles.invoicebtn} onClick={handleOrderStatusSubmit}>Update Order Status</Button>
                                    </div>
                                </div>
                                <div className="col-4 mt-3">
                                    <h6 className="">
                                    ...
                                    <p className="mt-2">{editData && editData[0] && editData[0].billing_address && editData[0].billing_address.name}                  </p>
                                    <p>{editData && editData[0] && editData[0].billing_address && editData[0].billing_address.address_type}:
                                        {editData && editData[0] && editData[0].billing_address && editData[0].billing_address.address}
                                    </p>
                                    <p>{editData && editData[0] && editData[0].billing_address && editData[0].billing_address.landmark}
                                    </p>
                                    <p>{editData && editData[0] && editData[0].billing_address && editData[0].billing_address.city} {" "}
                                        {editData && editData[0] && editData[0].billing_address && editData[0].billing_address.pincode}</p>
                                    <p>{editData && editData[0] && editData[0].billing_address && editData[0].billing_address.state}
                                    </p>
                                    <p className="mt-3">Email address:</p>
                                    <a href="#">
                                        <u>{editData && editData[0] && editData[0].billing_address && editData[0].billing_address.email}</u>
                                    </a>
                                    <p className="mt-3">Phone:</p>
                                    <a href="#">
                                        <u>{editData && editData[0] && editData[0].billing_address && editData[0].billing_address.mobile}</u>
                                    </a>
                                </div>
                                <div className="col-4 mt-3">
                                    <h6 className="">
                                    ...
                                    <p className="mt-2">{editData && editData[0] && editData[0].shipping_address && editData[0].shipping_address.name}                  </p>
                                    <p>{editData && editData[0] && editData[0].shipping_address && editData[0].shipping_address.address_type}:
                                        {editData && editData[0] && editData[0].shipping_address && editData[0].shipping_address.address}
                                    </p>
                                    <p>{editData && editData[0] && editData[0].shipping_address && editData[0].shipping_address.landmark}
                                    </p>
                                    <p>{editData && editData[0] && editData[0].shipping_address && editData[0].shipping_address.city} {" "}
                                        {editData && editData[0] && editData[0].shipping_address && editData[0].shipping_address.pincode}</p>
                                    <p>{editData && editData[0] && editData[0].shipping_address && editData[0].shipping_address.state}
                                    </p>
                                    <p className="mt-3">Email address:</p>
                                    <a href="#">
                                        <u>{editData && editData[0] && editData[0].shipping_address && editData[0].shipping_address.email}</u>
                                    </a>
                                    <p className="mt-3">Phone:</p>
                                    <a href="#">
                                        <u>{editData && editData[0] && editData[0].shipping_address && editData[0].shipping_address.mobile}</u>
                                    </a>
                                </div>
                            </div>
                        </Card>
                            ...
                            <Accordion defaultActiveKey="0" className="p-0">
                                <Accordion.Item eventKey="0">
                                    ...
                                    <Accordion.Body className="text-center">
                                        <Button className={styles.trackingbtn} onClick={handleTrackInfo}>Get Tracking Info</Button>
                                    </Accordion.Body>
                                </Accordion.Item>
                            </Accordion>
                            <Accordion defaultActiveKey="0" className="p-0">
                                <Accordion.Item eventKey="0">
                                    ...
                                    <Accordion.Body>
                                        <div className="d-flex justify-content-around">
                                            ...
                                            <Button className={styles.invoicebtn} onClick={handleInvoiceDownload}>Invoice Download</Button>
                                        </div>
                                    </Accordion.Body>
                                </Accordion.Item>
                            </Accordion>
                                ...
                                <Card className={styles.card}>
                                    <Form.Group>
                                        ...
                                        <table className="table table-striped mt-3">
                                            <thead>
                                                <tr>
                                                    <th style={{ backgroundColor: "#004a98" }} >Order ID</th>
                                                    <th style={{ backgroundColor: "#004a98" }}>Image</th>
                                                    <th style={{ backgroundColor: "#004a98" }}>Product Name</th>
                          ...
                          editData.map((productName) => (
                            <tr key={productName.product_id}>
                              ...
                              <td> ₹ {Number(productName.product_price) * Number(productName.quantity)}</td>
                                                ...
                                                {editData && editData.map(item => {
                                                    ...
                                                    return (
                                                        <tr key={item.product_id}>
                                                            ...
                                                            <td>
                                                                {imgeInfo && imgeInfo.map((url, index) => (
                                                                    <img key={index} src={url.image} alt={item.name} width="50" height="50" style={{ margin: '0 5px' }} />
                                                                ))}
                                                            </td>
                                                            <td>{item.product_name}</td>
                                                            <td>{item && item.final_price}</td>
                                                            <td>{item.quantity}</td>
                                                            <td> ₹ {Number(item && item.final_price) * Number(item.quantity)}</td>
                                                        </tr>
                                                    );
                                                })}
                                            </tbody>
                                    ...
                                    <Form.Group>
                                        <Form.Label style={{ color: "#ff9800", marginright: "20" }}>Item Subtotal:  </Form.Label>
                                        {" "} ₹ {Number(total && total.totalAmount).toFixed(2)}
                                    </Form.Group>
                                    {editData && editData[0] && editData[0].discount &&
                                        <Form.Group>
                                            <Form.Label style={{ color: "#ff9800", marginright: "20" }}>Coupon Discount: {"("}{editData && editData[0] && editData[0].coupon_code}{")"} </Form.Label>
                                            {" "} -₹ {Number(editData && editData[0] && editData[0].discount).toFixed(2)}
                                        </Form.Group>
                                    }
                                    {total && total.totalAmount > 599 && editData && editData[0] && editData[0].is_cod != true &&
                                        <Form.Group>
                                            <Form.Label style={{ color: "#ff9800", marginright: "20" }}>1% Discount on above 599: </Form.Label>
                                            {" "} -₹ {Number(total && total.totalAmount * 0.01).toFixed(2)}
                                        </Form.Group>
                                    }
                                    {editData && editData[0] && editData[0].gift_wrapping == true &&
                                        <Form.Group>
                                            <Form.Label style={{ color: "#ff9800", marginright: "20" }}>Gift Wrapping: </Form.Label>
                                            {" "} ₹ {Number(40).toFixed(2)}
                                        </Form.Group>
                                    }
                                    {editData && editData[0] && editData[0].shippingCharge &&
                                        <Form.Group>
                                            <Form.Label style={{ color: "#ff9800", marginright: "20" }}>Shipping Charge: </Form.Label>
                                            {" "} ₹ {Number(editData && editData[0] && editData[0].shippingCharge).toFixed(2)}
                                        </Form.Group>
                                    }
                                    {editData && editData[0] && editData[0].walletMoneyUsed > 0 &&
                                        <Form.Group>
                                            <Form.Label style={{ color: "#ff9800", marginright: "20" }}>Wallet Money Used:  </Form.Label>
                                            {" "} ₹ {Number(editData && editData[0] && editData[0].walletMoneyUsed).toFixed(2)}
                                        </Form.Group>
                                    }
                                    <Form.Group>
                                        <Form.Label style={{ color: "#ff9800", marginright: "20" }}>Amount Paid:  </Form.Label>
                                        {" "} ₹ {Number(editData && editData[0] && editData[0].subtotal).toFixed(2)}
                                    </Form.Group>

                                    <Form.Group>
                                        <Form.Label style={{ color: "#ff9800", marginright: "20" }}>Total Amount:  </Form.Label>
                                        {" "} ₹ {(Number(editData && editData[0]?.subtotal || 0) + Number(editData && editData[0]?.walletMoneyUsed || 0)).toFixed(2)}
                                    </Form.Group>
                                </Card>
                            )}
                        </Card>
                    ...
                    {/* Billing popup */}
                    <Modal id="" show={billingModalPopupShow} onHide={handleClosePopup} size="lg">
                        <Modal.Header closeButton>
                        </Modal.Header>
                        <Form onSubmit={handleBillingAddressSubmit} className={styles.formItems}>
                            ...
                            <div style={{ width: "100px", textAlign: "center", marginBottom: "20px" }}>
                                <Button
                                    type="submit"
                                >
                                    Submit
                    ...
                    {/* shippinh popup */}
                    <Modal id="" show={shippingModalPopupShow} onHide={handleCloseShippingPopup} size="lg">
                        <Modal.Header closeButton>
                        </Modal.Header>
                        <Form onSubmit={handleShippingAddressSubmit} className={styles.formItems}>
                            <div style={{ textAlign: "", padding: "10px" }} >
                                <Form.Group>
                    ...
                    {/* Track Info Modal */}
                    <Modal id="tag-people" size="md"
                        // style={{ marginTop: '50px' }}
                        show={showTrackInfo} onHide={handleCloseTrackInfo}>
                        <Modal.Header>
                            <Modal.Title>Tracking Information</Modal.Title>
                            <button type="button" onClick={handleCloseTrackInfo} className="close" data-dismiss="modal" aria-hidden="true">
                                ×
                            </button>
                                ...
                                {trackInfoData?.scans?.length > 0 ? (
                                    trackInfoData.scans.map((item, index) => (
                                        <div className="d-flex" key={index} id="tracking">
                                            ...
                                            <div>
                                                <p className={styles.orderConfirm}>{item['sr-status-label']}</p>
                                                <p className={styles.confirmDate}>{trackerFormatDate(item.date)}</p>
                                            </div>
                                        </div>
                                    ))
                                ) : (
