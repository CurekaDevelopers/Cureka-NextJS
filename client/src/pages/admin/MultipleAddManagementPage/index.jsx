...
const MultipleAddManagementPage = () => {
    ...
    if (multipleAddToDelete?.id && multipleAdds?.length) {
      dispatch(
        deleteMultipleAdd(multipleAddToDelete.id, () => {
          const updatedMultipleAddList = multipleAdds.filter((item) => item.id !== multipleAddToDelete.id);
  ...
  const onEdit = useCallback((id) => {
    navigate(pagePaths.adminCreateMultipleAddEdit.replace(":id", id));
  }, [navigate]);
  const rolesPermission = userRoles.filter(item => item.name == "Multiple Add Management")
        ...
        <div>
          <Card className={styles.card}>
            <div className={styles.cardHeader}>
              ...
              <div>
                {isAdminStatus == 1 ? (
                  <Link to={pagePaths.adminCreateMultipleAdd}>
