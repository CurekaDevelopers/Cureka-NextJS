...
const AdminRatingAndReviewPage = () => {
  ...
  const rolesPermission = userRoles.filter(item => item.name == "RatingAndReview")
          ...
          <div>
            {isAdminStatus == 1 ? (
              <div className="d-flex">
                {status !== "Approved" && (
                  <AcceptButton onClick={() => onEdit(cell.row.original.id, "Approved")} />
                )}
                {status === "Approved" && (
                  <RejectButton onClick={() => onEdit(cell.row.original.id, "Rejected")} />
                )}
              </div>
            ) : (
              <>
  ...
  const mappedRateAndReview = useMemo(() => {
    ...
    return ratingAndReviews.filter(item =>
      item && item.vendor_article_name?.toLowerCase().includes(searchTerm.toLowerCase())
      ...
      <div>
        ...
        <Card className={styles.card}>
          ...
          {mappedRateAndReview?.length ? (
            ...
            <p className={styles.noRecordFoundMessage}>No Rating and Reviews found, please add new blog.</p>
          )}
        </Card>
      </div>
    </div>
