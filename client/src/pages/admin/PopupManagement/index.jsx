...
import { setAdminEmail, setIsLoggedIn, setUserRoles } from "../../../../src/redux/slices/auth.slice";
...
const PopupManagementPage = () => {
  ...
  const onEdit = useCallback((id) => {
    navigate(pagePaths.adminPopupManagementEdit.replace(":id", id));
  }, [navigate]);
    ...
    if (popupToDelete?.id && adminPopupList?.length) {
      dispatch(
        deletePopup(popupToDelete.id, () => {
          const updatedPopupList = adminPopupList.filter((item) => item.id !== popupToDelete.id);
  ...
  const rolesPermission = userRoles.filter(item => item.name == "Popup Management")
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      Cell: ({ cell }) => {
        return <span className={styles.titleColumn}>{cell.row.original.coupon_code?cell.row.original.coupon_code:"No Coupons"}</span>;
      },
    },
    {
      Header: "Count Down",
      ...
      <div>
        <Card className={styles.card}>
          <div className={styles.cardHeader}>
            ...
            <div>
              {isAdminStatus == 1 ? (
                <Link to={pagePaths.adminPopupManagementAdd}>
          ...
          {mappedCustomerOrders?.length ? (
            ...
            <p className={styles.noRecordFoundMessage}>No Popup found, please add new Popup.</p>
          )}
        </Card>
      </div>
      <ConfirmationModel
