    ...
    if (concernToDelete?.id && concerns?.length) {
      dispatch(
        deleteConcerns(concernToDelete.id, () => {
          const updatedConcernsList = concerns.filter((item) => item.id !== concernToDelete.id);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "image",
        Cell: ({ cell }) => {
          return <img style={{ height: 50, width: 50 }} src={cell.row.original.product_images&&
            cell.row.original.product_images[0]&&cell.row.original.product_images[0].image} />;
        },
      },
      {
        Header: "Product Name",
        ...
        Cell: ({ cell }) => {
          return <span className={styles.titleColumn}>{cell.row.original.vendor_article_name}</span>;
        },
      },
      {
        Header: "Name",
        accessor: "first_name",
        Cell: ({ cell }) => {
          return <span className={styles.titleColumn}>{cell.row.original.first_name}</span>;
        },
      },
      {
        Header: "Mobile",
        ...
        Cell: ({ cell }) => {
          return <span className={styles.titleColumn}>{"+91"} {cell.row.original.mobile_number}</span>;
        },
      },
      {
