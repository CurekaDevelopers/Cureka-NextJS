...
const CouponsManagementPage = () => {
    ...
    if (couponToDelete?.id && coupons?.length) {
      dispatch(
        deleteCoupons(couponToDelete.id, () => {
          const updatedConcernsList = coupons.filter((item) => item.id !== couponToDelete.id);
  ...
  const onEdit = useCallback((id) => {
    navigate(pagePaths.adminCreateCouponEdit.replace(":id", id));
  }, [navigate]);
  const rolesPermission = userRoles.filter(item => item.name == "Coupons Management")
        ...
        <div>
          <Card className={styles.card}>
            <div className={styles.cardHeader}>
              ...
              <div>
                {isAdminStatus == 1 ? (
                  <Link to={pagePaths.adminCreateCoupon}>
