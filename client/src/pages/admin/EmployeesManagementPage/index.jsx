...
import { deleteEmployee, fetchEmployees, fetchAllRoles } from "../../../redux/action";
...
const EmployeesManagementPage = () => {
  ...
  const { roles, isAdminStatus, userRoles } = useSelector((state) => state.auth);
    ...
    if (employeeToDelete?.id && employees?.length) {
      dispatch(
        deleteEmployee(employeeToDelete.id, () => {
          const updatedEmployeesList = employees.filter((item) => item.id !== employeeToDelete.id);
  ...
  const onEdit = useCallback((id) => {
    navigate(pagePaths.adminCreateEmployeeEdit.replace(":id", id));
  }, [navigate]);
  const getRoleNames = (rolesArray, rolesOfEmployee) => {
    ...
    const uniqueRoleNames = [...new Set(roleNames)].filter(name => name !== null);

    return uniqueRoleNames.join(', ');
  };
  const rolesPermission = userRoles.filter(item => item.name == "Employees Management")
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      accessor: "name",
      Cell: ({ cell }) => {
        return <span className={styles.titleColumn}>{`${cell.row.original.first_name} ${cell.row.original.last_name}`}</span>;
      },
    },
    {
      Header: "Roles",
      accessor: "roles",
      Cell: ({ cell }) => {
        return <span className={styles.titleColumn}>{getRoleNames(roles, cell.row.original.roles)}</span>;
      },
    },
    {
      Header: "Created By",
        ...
        <div>
          <Card className={styles.card}>
            <div className={styles.cardHeader}>
              ...
              <div>
                {isAdminStatus == 1 ? (
                  <Link to={pagePaths.adminCreateEmployee}>
