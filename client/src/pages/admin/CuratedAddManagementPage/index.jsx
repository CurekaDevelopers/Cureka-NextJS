...
const CuratedAddManagementPage = () => {
  const { curatedAdds, curatedAdds: { CURATED } } = useSelector((state) => state.admin);
  ...
  const onDeleteConfirmed = () => {
    if (curatedAddToDelete?.id && CURATED?.length) {
      dispatch(
        deleteCuratedAdd(curatedAddToDelete.id, () => {
          const updatedCuratedAddList = CURATED.filter((item) => item.id !== curatedAddToDelete.id);
  ...
  const onEdit = useCallback((id) => {
    navigate(pagePaths.adminCreateCuratedAddEdit.replace(":id", id));
  }, [navigate]);
  const handleUpload = (id) => {
    window.open(`/admin/curated/upload/images/${id}`, '_blank');
  };
  const rolesPermission = userRoles.filter(item => item.name == "Curated Add Management")
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      accessor: "image",
      Cell: ({ cell }) => {
        return <img style={{ height: 50, width: 50 }} src={cell.row.original.image} />;
      },
    },
    {
      Header: "Url",
        ...
        return (
          ...
          {isAdminStatus == 1 ? (
            <div>
              ...
              <button className={styles.upload} onClick={() => handleUpload(cell.row.original.id)}>
            ...
            <>
              {rolesPermission && rolesPermission.map(role => (
                <div key={role.roleId}>
                  ...
                  {role.isUpdate == 1 && <button className={styles.upload} onClick={() => handleUpload(cell.row.original.id)}>
                    <FontAwesomeIcon icon={faUpload} />
                  </button>}
                </div>
              ))}
        ...
        <div>
          <Card className={styles.card}>
            <div className={styles.cardHeader}>
              ...
              <div>
                {isAdminStatus == 1 ? (
                  <Link to={pagePaths.adminCreateCuratedAdd}>
