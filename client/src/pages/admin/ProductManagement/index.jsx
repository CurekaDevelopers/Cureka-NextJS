...
const ProductManagementPage = () => {
    ...
    if (brandToDelete?.id && productsList?.length) {
      dispatch(
        deleteProducts(brandToDelete.id, () => {
          const updatedBlogsList = productsList?.filter((item) => item.id !== brandToDelete.id);
  ...
  const onEdit = useCallback(
    (id, replay = 'edit') => {
      if (replay === 'faq') {
        navigate(pagePaths.adminCreateProductSize.replace(":id", id));
        return
      }
      navigate(pagePaths.adminCreateProductEdit.replace(":id", id));
    },
    [navigate],
  );
  ...
  const rolesPermission = userRoles.filter(item => item.name == "Products Management")
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "name",
        Cell: ({ cell }) => {
          return <span className={styles.titleColumn}>{cell.row.original.name}</span>;
        },
      },
      {
        Header: "Status",
            ...
            <div>
              {isAdminStatus == 1 ? (
                <div>
                  ...
                  <ReplayButton onClick={() => onEdit(cell.row.original.id, 'faq')} />
                  <DeleteButton onClick={() => onDeleteButtonClicked(cell.row.original)} />
                </div>
              ) : (
                <>
                  {rolesPermission && rolesPermission.map(role => (
                    <div key={role.roleId}>
                      ...
                      {role.isUpdate == 1 && <ReplayButton onClick={() => onEdit(cell.row.original.id, 'faq')} />}
                      {role.isDelete == 1 && <DeleteButton onClick={() => onDeleteButtonClicked(cell.row.original)} />}
                    </div>
                  ))}
                </>
              )}
  ...
  const filteredBrands = useMemo(() => {
    ...
    return productsList
      .filter(item => {
        ...
        return (
          (item.vendor_article_name && item.vendor_article_name.toLowerCase().includes(searchTermLower)) ||
          (item.product_id && item.product_id.toString().includes(searchTermLower)) ||
          ...
          <Card className={styles.card}>
            ...
            <div className="d-flex">
              ...
              <div className="ml-auto">
                ...
                <Link to={pagePaths.adminReportExcel}>
                  <Button className={styles.addButton}>Report Excel</Button>
                </Link>
                {isAdminStatus == 1 ? (
                  <Link to={pagePaths.adminCreateProducts}>
            ...
            {filteredBrands.length ? (
              ...
              <p className={styles.noRecordFoundMessage}>Loading products, please wait...</p>
            )}
          </Card>
        </div>
      </div>
