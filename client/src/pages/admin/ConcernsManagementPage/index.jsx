...
const ConcernsManagementPage = () => {
    ...
    if (concernToDelete?.id && adminConcerns?.length) {
      dispatch(
        deleteConcerns(concernToDelete.id, () => {
          const updatedConcernsList = adminConcerns.filter((item) => item.id !== concernToDelete.id);
  ...
  const onEdit = useCallback(
    (id) => {
      navigate(pagePaths.adminCreateConcernEdit.replace(":id", id));
    },
    [navigate],
  );
  const rolesPermission = userRoles.filter(item => item.name == "Concerns Management")
          ...
          <Card className={styles.card}>
            ...
            <div className="d-flex">
              ...
              <div className="ml-auto">
                {isAdminStatus == 1 ? (
                  <Link to={pagePaths.adminCreateConcern}>
