...
const SelfAddManagementPage = () => {
  const { curatedAdds, curatedAdds: { YOURSELF } } = useSelector((state) => state.admin);
    ...
    if (selfAddToDelete?.id && YOURSELF?.length) {
      dispatch(
        deleteCuratedAdd(selfAddToDelete.id, () => {
          const updatedSelfAddList = YOURSELF.filter((item) => item.id !== selfAddToDelete.id);
  ...
  const onEdit = useCallback((id) => {
    navigate(pagePaths.adminCreateSelfAddEdit.replace(":id", id));
  }, [navigate]);
  const rolesPermission = userRoles.filter(item => item.name == "Self Add Management")
        ...
        <div>
          <Card className={styles.card}>
            <div className={styles.cardHeader}>
              ...
              <div>
                {isAdminStatus == 1 ? (
                  <Link to={pagePaths.adminCreateSelfAdd}>
                  ...
                  <>
                    {rolesPermission && rolesPermission.map(role => (
                      <div key={role.roleId}>
                        {role.isAdd == 1 && <Link to={pagePaths.adminCreateSelfAdd}>
                          <Button className={styles.addButton}>
                            <MdAdd /> Add Self Add
                          </Button>
                        </Link>
