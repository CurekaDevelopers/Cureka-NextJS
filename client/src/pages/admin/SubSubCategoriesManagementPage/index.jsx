...
import { deleteSubSubCategories, fetchSubsubCategories } from "../../../redux/action";
...
const SubSubSubCategoriesManagementPage = () => {
  ...
  const onEdit = useCallback(
    (id) => {
      navigate(pagePaths.adminCreateSubSubCategoryEdit.replace(":id", id));
    },
    [navigate],
  );
  ...
  const rolesPermission = userRoles.filter(item => item.name == "Sub Sub Category")
  ...
  <div className="ml-auto">
              <Button className={styles.addButton} onClick={exportToExcel}>Export to Excel</Button>
                {isAdminStatus == 1 ? (
                  <Link to={pagePaths.adminCreateSubSubCategory}>
