...
const OrderManagementPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchAdminOrderList = async () => {
      try {
        const response = await axios.get(`${env.REACT_SERVER_BASE_URL}/api/v1/checkout/adminOrders`);
        setAdminOrderList(response && response.data)
      } catch (error) {
        console.error('Error fetching data:', error);
        // Handle error state or notify user
  ...
  const onEdit = useCallback((id) => {
    navigate(pagePaths.adminEditOrder.replace(":id", id));
  }, [navigate]);
  ...
  const rolesPermission = userRoles.filter(item => item.name == "Order Management")
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      accessor: "created_at",
      Cell: ({ cell }) => {
        return <span className={styles.titleColumn}>{moment(cell.row.original.created_at).format('YYYY-MM-DD HH:mm:ss')}</span>;
      },
    },
    {
      Header: "Status",
    ...
    },
    {
      ...
      accessor: "first_name",
      Cell: ({ cell }) => {
        return <span className={styles.titleColumn}>{cell.row.original.shipping_address.name}</span>;
      },
    },
    {
      Header: "Coupons",
      ...
      Cell: ({ cell }) => {
        return (
          <>{cell.row.original.coupon_code ? cell.row.original.coupon_code : "No Coupons"}</>
        );
      },
    },
    // {
    ...
    },
    {
      ...
      Cell: ({ cell }) => {
        return (
          <>₹ {Number(cell.row.original.walletMoneyUsed).toFixed(2)}</>
        );
      },
    },
    {
      ...
      Cell: ({ cell }) => {
        return (
          <>₹ {Number(cell.row.original.subtotal).toFixed(2)}</>
        );
      },
    },
    {
      ...
      Cell: ({ cell }) => {
        return (
          <div>
            {isAdminStatus == 1 ? (
              <div>
                <EditButton onClick={() => onEdit(cell.row.original.order_id)} />
                {cell.row.original.order_placed_status !== "Cancelled" ? <RejectButton onClick={() => handleCancelOrder(cell.row.original.order_id, 'Cancel Order')} /> : ""}
              </div>
            ) : (
              <>
                {rolesPermission && rolesPermission.map(role => (
                  <div key={role.roleId}>
                    {role.isUpdate == 1 && <EditButton onClick={() => onEdit(cell.row.original.id)} />}
                    {cell.row.original.order_placed_status !== "Cancelled" ? role.isDelete == 1 && <RejectButton onClick={() => handleCancelOrder(cell.row.original.order_id, 'Rejected')} /> : ""}
                  </div>
                ))}
              </>
            )}
  ...
  const mappedCustomerOrders = useMemo(() => {
    ...
    return adminOrderList.filter(item =>
      item.shipping_address.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      (item.order_id && item.order_id.toString().includes(searchTerm)) ||
      item.coupon_code?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      item.order_placed_status?.toLowerCase().includes(searchTerm.toLowerCase())
    );
        ...
        <Card className={styles.card}>
          ...
          <div className="d-flex">
            ...
            <div className="ml-auto">
              <Button className={styles.addButton} onClick={exportToExcel}>Export to Excel</Button>
              {isAdminStatus == 1 ? (
                <Link to={pagePaths.adminCreateOrder}>
          ...
          {mappedCustomerOrders?.length ? (
            ...
            <p className={styles.noRecordFoundMessage}>No orders found, please add new Orders.</p>
          )}
          ...
          <Modal show={orderCancelClose} onHide={handleOrderCancelClose} id="rateproductModal">
            ...
            <Modal.Body>
              ...
              <div className="submit-btn">
                <Button onClick={handleSubmitCancelOrder} className={styles.addButton}>
                  Submit
                </Button>
              </div>
            </Modal.Body>
