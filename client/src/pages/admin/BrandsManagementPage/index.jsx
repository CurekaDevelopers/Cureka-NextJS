...
const BrandsManagementPage = () => {
    ...
    if (brandToDelete?.id && brands?.length) {
      dispatch(
        deleteBrands(brandToDelete.id, () => {
          const updatedBlogsList = brands.filter((item) => item.id !== brandToDelete.id);
  ...
  const onEdit = useCallback(
    (id) => {
      navigate(pagePaths.adminCreateBrandEdit.replace(":id", id));
    },
    [navigate],
  );
  ...
  const rolesPermission = userRoles.filter(item => item.name == "Brands Management")
        ...
        <div>
          <Card className={styles.card}>
            ...
            />
             <div className="ml-auto">
              <Button className={styles.addButton} onClick={exportToExcel}>Export to Excel</Button>
                {isAdminStatus == 1 ? (
                  <Link to={pagePaths.adminCreateBrand}>
            ...
            {mappedBrands?.length ? (
              ...
              <p className={styles.noRecordFoundMessage}>No brands found, please add new brand.</p>
            )}
          </Card>
        </div>
      </div>
