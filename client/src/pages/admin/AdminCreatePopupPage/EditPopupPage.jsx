...
import { fetchNestedCategories, fetchConcerns, fetchProductsOptions, fetchBrands } from "../../../redux/action";
...
const AdminEditPopupPage = () => {
  const { productOptions, concerns, brands, } = useSelector((state) => state.admin);
  ...
  useEffect(() => {
    ...
    const fetchDataActiveCoupons = async () => {
      try {
        const response = await axios.get(`${env.REACT_SERVER_BASE_URL}/coupons/getAllActiveCoupons`);
    ...
    const fetchEditData = async () => {
      try {
        const response = await axios.get(`${env.REACT_SERVER_BASE_URL}/popup/getPopup/${id}`);
        ...
        const filteredCategory = nestedCategories.filter(cat => filterCategoryIds.includes(cat.id));
        ...
        const filteredConcerns = concerns.filter(con => filterConcernsIds.includes(con.id));
        ...
        const filteredBrands = brands.filter(brands => filterBrandsIds.includes(brands.id));
  ...
  const couponOptions = useMemo(() => {
    ...
    const filteredCoupons = activecouponeList.find(coupon => coupon.coupon_code == filterCoupons);
  ...
  const onChangeCoupons = (event) => {
    const data = activecouponeList.find(item => item.id == event.target.value)
  ...
  const handleSubmit = async (event) => {
    ...
    let fileUrl = "";
    if (image) {
      const uploadData = await uploadImage(image, "ratingAndReviews", (uploadProgress) => {
    ...
    // return;
    try {
      const response = await axios.put(`${env.REACT_SERVER_BASE_URL}/popup/editPopup/${id}`, formData);
      let data = response.data;
      if (data) {
        toast.success("Popup Updated Successfully");
      }
      navigate(pagePaths.adminPopupManagement);
      ...
      <div>
        <Card className={styles.card}>
          ...
          <Form onSubmit={handleSubmit} className={styles.formItems}>
            ...
            <Form.Group>
              <Form.Label>Coupons<span className="text-danger">*</span></Form.Label>
              <Form.Select
                name="coupon"
              ...
              >
                ...
                {couponOptions.length > 0 && couponOptions.map((option) => (
            ...
            <Form.Group>
              ...
              {categoryOptions?.length > 0 && <MultiSelect
                value={categoryList}
            ...
            <Form.Group>
              ...
              {optionBrand?.length > 0 && <MultiSelect
                value={brandList}
                options={optionBrand}
                ...
                <div>
                  ...
                  <ul>
                    {Array.from(selectedProducts).map(value => (
                      <li key={value}>{options.find(option => option.value === value)?.label}</li>
                    ))}
                  </ul>
                </div>
              )}
