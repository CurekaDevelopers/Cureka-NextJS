    ...
    if (blogToDelete?.id && blogs?.length) {
      dispatch(
        deleteBlog(blogToDelete.id, () => {
          const updatedBlogsList = blogs.filter((item) => item.id !== blogToDelete.id);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        accessor: "title",
        Cell: ({ cell }) => {
          return <span className={styles.titleColumn}>{cell.row.original.title}</span>;
        },
      },
      {
        Header: "Date",
        accessor: "blog_date",
        Cell: ({ cell }) => {
          return <span>{dayjs(cell.row.original.blog_date).format("YYYY-MM-DD")}</span>;
        },
      },
      {
        Header: "Actions",
            ...
            <div>
              {isAdminStatus == 1 ? (
                <div>
                  ...
                  <ReplayButton onClick={() => onEdit(cell.row.original.id, 'replay')} />
                ...
                <>
                  {rolesPermission && rolesPermission.map(role => (
                    <div key={role.roleId}>
                      ...
                      {role.isUpdate == 1 && <ReplayButton onClick={() => onEdit(cell.row.original.id, 'replay')} />}
        ...
        <div>
          <Card className={styles.card}>
            ...
            {mappedBlogs?.length ? (
              ...
              <p className={styles.noRecordFoundMessage}>No blogs found, please add new blog.</p>
            )}
          </Card>
        </div>
      </div>
