...
const SingleAddManagementPage = () => {
    ...
    if (singleAddToDelete?.id && singleAdds?.length) {
      dispatch(
        deleteSingleAdd(singleAddToDelete.id, () => {
          const updatedSingleAddList = singleAdds.filter((item) => item.id !== singleAddToDelete.id);
  ...
  const onEdit = useCallback((id) => {
    navigate(pagePaths.adminCreateSingleAddEdit.replace(":id", id));
  }, [navigate]);
  const rolesPermission = userRoles.filter(item => item.name == "Single Add Management")
