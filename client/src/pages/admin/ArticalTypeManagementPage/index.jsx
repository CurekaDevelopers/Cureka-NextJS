...
const ArticalTypeManagementPage = () => {
    ...
    if (categoryToDelete?.id && results?.length) {
      dispatch(
        deleteArticleType(categoryToDelete.id, () => {
          ...
          dispatch(setArticleType({ results: updatedConcernsList, pagination: {} } || []));
  ...
  const rolesPermission = userRoles.filter(item => item.name == "Article Type Management")
            ...
            <div>
              {isAdminStatus == 1 ? (
                <div>
                  ...
                  <EditButton iconType='standard' onClick={() => navigate(pagePaths.adminStandardSize.replace(":id", cell?.row?.original?.id))} />
                </div>
              ) : (
                <>
                  {rolesPermission && rolesPermission.map(role => (
                    <div key={role.roleId}>
                      ...
                      {role.isUpdate == 1 && <EditButton iconType='standard' onClick={() => navigate(pagePaths.adminStandardSize.replace(":id", cell?.row?.original?.id))} />
                      } </div>
                  ))}
                </>
              )}
          ...
          <Card className={styles.card}>
            ...
            />
              <div className="ml-auto">
                {isAdminStatus == 1 ? (
                  <Link to={pagePaths.adminCreateArticalType}>
