...
export default function FaqsPage() {
  ...
  useEffect(() => {
    api.get(apiUrls.getAllFaqs).then((response) => {
  ...
  const faqSchema = faqs && typeof faqs === 'object' && Object.keys(faqs).length > 0 ? {
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Shop the best of health care products under one roof. Choose from a wide range of products. Best Prices. GreatDiscounts." />
        <link rel="canonical" href={window.location.href} />
        <meta property="og:url" content={window.location.href} />
        <meta property="og:type" content="website" />
        <meta property="og:title" content="Cureka" />
        <meta property="og:description" content="Shop the best of health care products under one roof. Choose from a wide range of products. Best Prices. GreatDiscounts." />
          ...
          <div className="row">
            <div className="col-lg-12">
              <Accordion activeKey={activeKey} id="faqaccord">
                {faqs && Object.keys(faqs).map((faqCategory, i) => (
                  <div key={i}>
                    ...
                    {faqs[faqCategory].map((item) => (
                      <Accordion.Item eventKey={item.id.toString()} key={item.id}>
                        ...
                        >
                          ...
                          <span className={`arrow-icon ${activeKey === item.id.toString() ? 'open' : ''}`}>
                            ▼
                          </span>
                        </div>
