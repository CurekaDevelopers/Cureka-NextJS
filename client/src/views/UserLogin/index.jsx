  ...
  const handleEmailPasswordLogin = async (values) => {
    try {
      ...
      const response = await api.post(apiUrls.emailLogin, {
        data: values.email, // Keeping as "data" per your backend
        password: values.password,
      });

      // Only handle success if access_token is present
      ...
      if (response.status === 200 && accessToken) {
        ...
        // Fetch user details
        const accountResponse = await api.get(apiUrls.getAccountDetails);
        const userDetails = _.get(accountResponse, "data.userDetails");

        if (userDetails) {
          dispatch(setUserDetails(userDetails));
        }

        // Close modal after login is fully successful
        setTimeout(() => {
          handleCloseLoginModel();
        }, 300);
      } else {
        // Even though 200, if no token — treat it as failure
      ...
      // Don't close modal here — let user correct credentials
    } finally {
      setLoading(false);
    }
  };
