...
import { Form, OverlayTrigger, ToggleButtonGroup, Tooltip } from "react-bootstrap";
...
export default function Cart() {
  ...
  const [isBillingAndShippingSame, setIsBillingAndShippingSame] = useState(true);
  const [selectedCoupon, setSelectedCoupon] = useState(null);
  const isMobile = useMediaQuery({ maxWidth: 767 });
  const dispatch = useDispatch();
  const { cartProducts, LastProducts, LikeProducts, cartPopup } = useSelector((state) => state.customer);
  const { coupons } = useSelector((state) => state.admin);
  const [discountInfo, setDiscountInfo] = useState(null);
  const [currentCheckoutStep, setCurrentCheckoutStep] = useState(checkoutStep.cart);
  const [manuallyEnteredCouponCode, setManuallyEnteredCouponCode] = useState("");
  ...
  const CustomToggleButton = ({ value, children, onClick, isSelected }) => (
    <button
      type="button"
      className={`text-decoration-none ${isSelected ? "reorder-btn" : "office-btn"} mr-0`}
  ...
  useEffect(() => {
    ...
    const fetchShippingCharges = async () => {
      try {
        ...
        const shippingChargesReponse = await axios.get(`${env.REACT_SERVER_BASE_URL}/api/v1/checkout/shippingCharges`);
        if (shippingChargesReponse) {
          setShippingChargesData(shippingChargesReponse && shippingChargesReponse.data && shippingChargesReponse.data.data)
        }
      } catch (error) {
  ...
  const onSubmitPayment =
    (isCod = false, isWalletOption = false) =>
      async (values) => {
        ...
        let walleAmount = walletMoney && walletMoney[0] && walletMoney[0].wallet_balance
        ...
        const data = {
          orderData: {
            ...
            billing_address_id: billingAddressId ? billingAddressId : shippingAddressId,
        ...
        if (response.order_id) {
          if (isCod || isWalletOption) {
            navigate(pagePaths.orderPlaced.replace(":orderId", response.order_id), { replace: true });
          } else {
            const sessionData = await getPaymentSessionId(response.order_id);
            if (sessionData?.sessionId) {
              const checkoutOptions = {
                paymentSessionId: sessionData?.sessionId,
                returnUrl: urlJoin(env.REACT_SERVER_BASE_URL, basePath, "status", "{order_id}"),
                cancelUrl: urlJoin(env.REACT_SERVER_BASE_URL, basePath, "status", "{order_id}"),
              };
              // Call the function
              initializeCashfree()
                .then((cashfree) => {
  ...
  const handlePinCodeBlur = () => {
    getAddressOnPincode(formikShippingAddress.values.pincode).then((response) => {
      if (response) {
  ...
  const handlePaymentSelection = (paymentMethod) => {
    ...
    const range = shippingChargesData.find(r => subTotalAmount >= r.min && subTotalAmount <= (r.max ?? Infinity));
  ...
  const handleFetchWalletAmount = async () => {
    ...
    if (token) {
      const response = await axios.get(`${env.REACT_SERVER_BASE_URL}/user/account/fetchwalletamount`, {
        headers: {
          'Content-Type': 'application/json',
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Cureka is one of the leading online healthcare store dealing with the best in class home health careproducts." />
        <link rel="canonical" href={window.location.href} />
        <meta property="og:url" content={window.location.href} />
        <meta property="og:type" content="website" />
        <meta property="og:title" content="Cureka" />
        <meta property="og:description" content="Cureka is one of the leading online healthcare store dealing with the best in class home health careproducts." />
      ...
      <div className="container-fluid px-0">
        ...
        >
          <Tab className="tab-pane-cart" eventKey={checkoutStep.cart} title="Cart">
              ...
              <div className="container">
                <div className="row">
                  <div className="col-lg-8">
                    ...
                    <a href={pagePaths.home} className="continue-shopping" target="_blank">
                      Continue Shopping
                    </a>
                                ...
                                {!!LastProducts?.length &&
                                  LastProducts?.map((product) => {
                                    ...
                                    if (product?.product_images) {
                                      ...
                                      product_front_na_image = product?.front_image
                                    }
                                    return (
                                        ...
                                        <div className="description">
                                          <div className="d-flex justify-content-center justify-content-lg-start">
                                            ...
                                            <a
                                              ...
                                              target="_blank"
                                              className=""
                                            ><p className="category">{product.name}</p></a>
                                          </div>

                                          <h2 className="toppicks-heading">{product.product_name}</h2>

                                          <p className="toppicks-discount">&#8377; {product.mrp}</p>

                                          <p className="toppicks-price">&#8377; {product.final_price}</p>
                  ...
                  <div className="col-lg-4">
                    ...
                    {/* <div className="address-three">
                      <div className="form-group mb-0 align-items-center d-flex" id="gift-wrapper">
                        <input type="text" className="manuallyEntryCouponCode" onChange={(e) => setManuallyEnteredCouponCode(e.target.value)} placeholder="Enter Coupon Code" />
                        <div>
                          <a href="javascript:void(0)" onClick={() => onCouponChange({ coupon_code: manuallyEnteredCouponCode }, true)} className="change align-self-center">
                            Apply Coupon
                          </a>
                        </div>
                      </div>
                      ...
                      <Dropdown>
                        <Dropdown.Toggle
                          ...
                          as={forwardRef(({ onClick, children }, ref) => (
                            <p onClick={onClick} ref={ref} className="coupon cursor-pointer">
                              <img
                        ...
                        >
                          <span style={{ color: discountInfo && discountInfo.discount > 0 ? 'green' : 'unset' }}>
                        ...
                        <Dropdown.Menu>
                          ...
                          {!!coupons?.length &&
                            coupons?.map(({ id, coupon_code, name }) => {
                              return (
                                <Dropdown.Item
                                  onClick={() => onCouponChange({ id, coupon_code, name })}
                                  key={id}
                      ...
                      <div className="address-three">
                        <div className="d-flex Address justify-content-start">
                          ...
                          <p className="deliveraddress mb-0">
                            Delivery to <span style={{ color: "#231F20" }}>500085, Hyderabad</span>
                          </p>
                    ...
                    {/* <div className="address-three">
                      <div className="form-group mb-0 align-items-center d-flex" id="gift-wrapper">
                        ...
                        />
                        <label className="gift-wrap cursor-pointer" htmlFor="billing">
                          Add Gift-Wrapping
                        </label>
                      </div>
                    </div> */}
          ...
          <Tab className="tab-pane-cart" eventKey={checkoutStep.delivery} title="Delivery Address" disabled>
                    ...
                    <div className="order-summary">
                      <div>
                                  ...
                                  <div className="address-item">
                                    <label htmlFor={address.id} className="address-label">
                                      <div className="address-column name-mobile d-flex">
                                        <div class="home-tab"><p class="mb-0">{address.address_type}</p></div>
                                      ...
                                      <div className="address-column">
                                        ...
                                        <p>
                                          {address.landmark}{address.city}, {address.state} - {address.pincode}</p>
                                      </div>
                                    </label>
                                  </div>
                                </div>
                        ...
                        <div className="d-flex justify-content-center">
                          <button onClick={handleAddAddress} className="text-decoration-none rate-btn btn mb-2"
                          >
                            <MdAdd /> Add Address
                          </button>
                          {!addAddressShow && selectedAddress !== null &&
                            <button onClick={onCartSelectedAddressContinueClicked} className="text-decoration-none rate-btn btn mb-2 ml-2">
                              <span className="text-decoration-none">Continue Payment</span>
                            </button>
                            ...
                            <div className="col-lg-6">
                              <div className="form-group">
                                ...
                                <input
                                  ...
                                  id="mobile_number"
                                  name="mobile_number"
                                  value={formikShippingAddress.values.mobile_number}
                                  onChange={(event) => {
                                ...
                                {formikShippingAddress.errors.mobile_number &&
                                  formikShippingAddress.touched.mobile_number && (
                                    <span className="error-text">
                                      {formikShippingAddress.errors.mobile_number}
                                    </span>
                                  )}
                            ...
                            <div className="col-lg-6">
                              <div className="form-group">
                                ...
                                <OverlayTrigger
                                  ...
                                  overlay={<Tooltip id="pincode-tooltip">"Please enter your 6-digit pincode, and the city and
                                    state will be filled in automatically."</Tooltip>}
                                >
                                  <input
                                    className="form-control"
                                    type="number"
                          ...
                          <div className="row mb-4">
                            <div className="col-lg-12">
                              <ToggleButtonGroup
                                ...
                                onChange={(value) =>
                                  formikShippingAddress.setFieldValue("address_type", value)
                                }
                                ...
                                {["Home", "Office", "Others"].map((item) => {
                                  return (
                                    <CustomToggleButton
                                      key={item}
                                      onClick={() => {
                                        formikShippingAddress.setFieldValue("address_type", item);
                                      }}
                                      isSelected={formikShippingAddress.values.address_type === item}
                                      value={item}
                                    >
                                      {item}
                                    </CustomToggleButton>
                            ...
                            <div className="col-lg-12">
                              <div className="form-group d-flex align-items-center">
                                <input
                                  ...
                                  onChange={() => setIsBillingAndShippingSame((p) => !p)}
                                />
                                <label
                            ...
                            <div className="col-lg-6">
                              <div className="form-group">
                                ...
                                <input
                                  ...
                                  id="mobile_number"
                                  name="mobile_number"
                                  value={formikBillingAddress.values.mobile_number}
                                  onChange={(event) => {
                                    const inputValue = event.target.value;
                                    const numericValue = inputValue.replace(/[^0-9]/g, "");
                                    event.target.value = numericValue.slice(0, 10);
                                ...
                                {formikBillingAddress.errors.mobile_number &&
                                  formikBillingAddress.touched.mobile_number && (
                                    <span className="error-text">
                                      {formikBillingAddress.errors.mobile_number}
                                    </span>
                                  )}
                          ...
                          <div className="row mb-4">
                            <div className="col-lg-12">
                              <ToggleButtonGroup
                                ...
                                onChange={(value) =>
                                  formikBillingAddress.setFieldValue("address_type", value)
                                }
                                className="d-flex gap-2"
                              >
                                {["Home", "Office", "Others"].map((item) => {
                                  return (
                                    <CustomToggleButton
                                      key={item}
                                      onClick={() => {
                                        formikBillingAddress.setFieldValue("address_type", item);
                                      }}
                                      isSelected={formikBillingAddress.values.address_type === item}
                                      value={item}
                                    >
                                      {item}
                                    </CustomToggleButton>
                      ...
                      {addAddressShow &&
                        ...
                        <div className="mt-3 ml-0">
                          <button
                            ...
                            href="#"
                            disabled={
                              formikBillingAddress.isSubmitting || formikShippingAddress.isSubmitting
                            }
                          >
                            Save & Delivery
                          </button>
          ...
          <Tab className="tab-pane-cart" eventKey={checkoutStep.payment} title="Payment" disabled>
            <div className="tab-pane" id="payment">
              <div className="container mt-5" id="shipping-address">
                <div className="row">
                  <div className="col-lg-8">
                    <div className="order-summary">
                      <p className="summary" style={{ marginBottom: "10px" }}>Select Payment Method</p>
                      ...
                      <div className="row">
                        <div className="d-flex mt-3 gap-4">
                          ...
                          <div>
                            <input
                              ...
                              name="paymentMethod"
                              value="makeapayement"
                              checked={selectedPaymentMethod === 'makeapayement'}
                              onChange={() => handlePaymentSelection('makeapayement')}
                              className="cursor-pointer"
                              disabled={isWalletMoneyChecked && walletMoney && walletMoney[0] && walletMoney[0].wallet_balance > finalAmount}

                            />
                            <label className="subtotal ml-2 " htmlFor="paymentMethod1">Make a Payment</label>
                          </div>
                          <div>
                            <input
                              type="radio"
                            ...
                            />
                            <label className="subtotal ml-2" htmlFor="paymentMethod2">Cash on Delivery</label>
                          </div>
                          ...
                          {selectedPaymentMethod === 'cashOnDelivery' && (
                            ...
                            >
                              <button
                                ...
                                className="text-decoration-none rate-btn btn"
                                // disabled={!isAmountValid}
                                disabled={!isAmountValid || (isWalletMoneyChecked && walletMoney && walletMoney[0] && walletMoney[0].wallet_balance > finalAmount)}
                        ...
                        {walletMoney && walletMoney[0] && walletMoney[0].wallet_balance > 0 &&
                          < div >
                            <input
                              type="checkbox"
                            ...
                            <label
                              ...
                              Wallet Money -  <span className="total-amount">₹
                                {walletMoney && walletMoney[0] && walletMoney[0].wallet_balance}</span>
                            </label>
                          </div>
                        }

                        {walletMoney && walletMoney[0] && walletMoney[0].wallet_balance > finalAmount &&
                  ...
                  <div className="col-lg-4">
                    <OrderSummary
                      ...
                      walletMoney={walletMoney && walletMoney[0] && walletMoney[0].wallet_balance}
