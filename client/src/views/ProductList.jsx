...
import { snakeCaseToTitleCase, sortFilterData } from "../utils/common.utils";
import { generateUrl } from "../utils/constants/common.constants";
import Footer from "./Footer";
import Header from "./Header";
import CategoryPopup from "./Header/HomePopup";
import ScrollToTop from './ScrollToTop';

import age1 from "../assets/images/0-5.png";
import age3 from "../assets/images/18-50.png";
import age2 from "../assets/images/5-18.png";
import age4 from "../assets/images/50+.png";


export default function ProductList() {
  const { categorySlug, subCategorySlug, subSubCategorySlug, brandSlug, productSlug } = useParams();
  ...
  const getAgeImage = () => {
    ...
    return skinbanner; // default fallback
  };
  useEffect(() => {
    const handleScroll = () => {
  ...
  //     (product) => product.final_price >= min && product.final_price <= max
  //   );
  // };
  // const handlePriceRanageChange = (newValues) => {
  //   setPriceRange(newValues);
  //   // setFilterProducts(filterProductsByPrice(newValues[0], newValues[1]));
  // };
  // Update price range values without calling the API immediately
  ...
  useEffect(() => {
    ...
    const filteredProducts = priceRange.length > 0
      ? products.filter(
        (product) =>
  ...
  useEffect(() => {
    if (nestedCategories.length) {
      const category = nestedCategories.find((item) => item?.slug === categorySlug);
      let subCategory = null;
      let subSubCategory = null;
      if (category?.sub_categories?.length) {
        subCategory = category?.sub_categories.find((item) => item.slug === subCategorySlug);
  ...
  useEffect(() => {
    let slugParams = { categorySlug, subCategorySlug, subSubCategorySlug, productSlug }
    ...
    if (location.pathname.includes('product-brands')) {
      dispatch(fetchProductsForBrand({ brandSlug }));

      return
    }
    if (location.pathname.includes('top-picks')) {
      dispatch(fetchProducts({ ...queryParamsObject, slugParams, url: 'top-picks' }));
      return
    }
    if (location.pathname.includes('new-arrivals')) {
      dispatch(fetchProducts({ ...queryParamsObject, slugParams, url: 'new-arrivals' }));
      return
    }
    if (location.pathname.includes('shop-by-age')) {
      let minAge, maxAge

      dispatch(fetchProducts({ ...queryParamsObject, slugParams, url: 'shop-by-age' }));
      return
    }
    dispatch(fetchProducts({ ...queryParamsObject, slugParams }));
  }, [dispatch, searchParams, categorySlug, subCategorySlug, subSubCategorySlug, brandSlug, productSlug]);
  ...
  const handlePaginationFilter = (type, filterKey) => {
    let searchData
    if (searchBrand) {
      searchData = filter[filterKey]?.filter((val) => val.value.toLowerCase().includes(searchBrand.toLowerCase()))
    }
    else {
  ...
  const handleFiltersSearch = (word, type) => {
    const searchData = filter[type]?.filter((val) => val.value.toLowerCase().includes(word.toLowerCase()))
    setSearchBrand(word)
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content={stripHtmlTags(catadata?.description)}></meta>
        <link rel="canonical" href={window.location.href} />
        <meta property="og:url" content={window.location.href} />
        <meta property="og:type" content="website" />
        <meta property="og:title" content="Cureka" />
        <meta property="og:description" content={stripHtmlTags(catadata?.description)} />
        <meta property="og:image" content="https://frontend.cureka.com/src/assets/images/logo.svg" />
        <script type="application/ld+json">
          {JSON.stringify(cschema)}
        </script>
      </Helmet>
      <div className="container-fluid px-0">
        ...
        <div className="container">
          <div className="d-flex home-back-section">
            ...
            <Link
              to={`/product-category/${categoryData?.category?.slug}`}
              className="section mb-0 ml-2"
            >

              / <span className="ml-2">{capitalizeFirstLetter(categoryData?.category?.name) || "Search result"}</span>
            </Link>
            {categoryData.subCategory?.slug && (
              <Link
                to={`/product-category/${categoryData?.category?.slug}/${categoryData?.subCategory?.slug}`}
                className="section mb-0 ml-2"
              >
                /<span className="ml-2">{capitalizeFirstLetter(categoryData?.subCategory?.name) || "Search result"}</span>
              </Link>
            )}
            {categoryData.subSubCategory?.slug && (
              <Link
                to={`/product-category/${categoryData?.category?.slug}/${categoryData?.subCategory?.slug}/${categoryData?.subSubCategory?.slug}`}
                className="section mb-0 ml-2"
              >
                /
                <span className="ml-2">{capitalizeFirstLetter(categoryData?.subSubCategory?.name) || "Search result"}</span>
              </Link>
            )}
          </div>
        </div>
          ...
          {!loading ? (
            <div className="skin-care">
              <div className="d-lg-flex">
                  ...
                  {!!Object.keys(filter || {})?.length &&
                    Object.keys(filter)
                      ...
                      .map((filterKey) => {
                        return (
                          <div key={filterKey}>
                            <Accordion defaultActiveKey="{null}" className="p-0">
                              <Accordion.Item eventKey="0">
                                <Accordion.Header as="div" className="collapsible-link heading-button">
                                  {snakeCaseToTitleCase(filterKey)}
                                </Accordion.Header>
                                <Accordion.Body className="p-0">
                                  <ul className="accor-space list-unstyled mt-3">
                                    {
                                      filterData && filterData[filterKey]?.length > 0 ?
                                        filterData[filterKey].slice(0, 10)?.map(({ value, count }) => {
                                          return (
                                            <li key={value}>
                                              <a
                                              ...
                                              >
                                                <label className="selectall">
                                                  <input
                                                    type="checkbox"
                                                    checked={isFilterSet(filterKey, value)}
                                                    onClick={handleCategorySelect(filterKey, value)}
                                                    className="select-input"
                                                  />
                                                  <span className="category-input">{value}</span>
                                                  <div className="selectall-num">{count}</div>
                                                </label>
                                              </a>
                                            </li>
                                          );
                                  ...
                                  {filterKey === 'brand' && <div style={{ display: 'flex', justifyContent: 'space-between', padding: 10 }}>
                                    {filter[filterKey]?.length > 10 && paginate > 1 && <p onClick={() => handlePaginationFilter('prev', filterKey)}>Prev</p>}

                                    {filter[filterKey]?.length > 10 && Math.ceil(filter[filterKey].length / 10) > paginate && <p onClick={() => handlePaginationFilter('next', filterKey)}>Next</p>}
                ...
                <div className="col-lg-12 order-2 d-lg-none d-block" id="category-filter">
                  {!!Object.keys(filter || {})?.length &&
                    Object.keys(filter)
                      .filter((item) => filter[item]?.length)
                      .map((filterKey) => {
                                      ...
                                      <div className="tab-vertical p-2">
                                        ...
                                        <Col sm={10} className="px-0">
                                          <Tab.Content className="mb-4">
                                            {filterData && typeof filterData === 'object' && Object.keys(filterData).map((filterKey) =>
                                                  ...
                                                  {filterKey === "brand" &&
                                                    filter[filterKey]?.length > 10 && (
                                                      ...
                                                      >
                                                        ...
                                                        {Math.ceil(filter[filterKey].length / 10) >
                                                          paginate && (
                                                            <p
                                                              onClick={() =>
                                                                handlePaginationFilter(
                ...
                <div
                  className={`col-lg-10 skin-banners order-1 order-lg-2 ${isFixed ? 'skin-banners with-margin' : 'skin-banners'}`} id="skin-banners"
                >
                  <div className="d-flex justify-content-between">
                    <div className="skin">
                      <h1 className="skin-heading">{capitalizeFirstLetter(catadata?.name) || "Search result"}</h1>
                    ...
                    <div className="d-lg-block d-none">
                      ...
                      <DropdownButton id="dropdown-toggle" title={`Sort by : ${sortByValue || ''}`}>
                        {sortFilterData?.map((filter) => {
                          return (
                          ...
                          filterProducts.map((product, index) => {
                            ...
                            return (
                              ...
                              <>
                                ...
                                {filterProducts?.length > 5 && index === 5 && <div className="mb-3">
                                  <img
                              ...
                              {products?.length < 5 && <div className="mb-3">
                                <img
                                  className="w-100 d-block img-fluid mx-auto"
                                  itemID={2}
                                  src={skinbanner}
                            ...
                            products.map((product, index) => {
                              ...
                              return (
                                ...
                                <>
                                  <div className="col-lg-4  col-md-4 col-6 mb-3">
                                    <div className="arrivals card">
                                      <div className="d-flex flex-nowrap justify-content-between">
                                        <div className="sale d-lg-block d-none">
                                          {product.discount_percent > 0 ? (
                                            <h2 className="sale-heading">-{Number(product.discount_percent)} % </h2>

                                          ) : (
                                            <div style={{ backgroundColor: product.discount_amount !== 0 ? 'sale-heading' : 'white' }}>
                                              {product.discount_amount !== 0 ? (
                                                <h2 className="sale-heading">{product.discount_amount} ₹</h2>
                                              ) : (
                                                <h2 className="sale-heading">{product.discount_amount}</h2>
                                              )}
                                            </div>
                                      ...
                                      >
                                        ...
                                        {
                                          product?.back_order_quantity == 0 || product && product.stock_status == "Out Stock"
                                            ? (
                                              <>
                                                <div className="d-flex-column pb-0 pb-lg-5">
                                                  <p className="text-center"
                                                    style={{
                                                      color: product?.back_order_quantity == 0 || product && product.stock_status == "Out Stock"
                                                        ? 'red' : '', fontSize: "10px"
                                                    }}
                                                  >
                                                    {product?.back_order_quantity == 0 || product && product.stock_status == "Out Stock"
                                              ...
                                              <>
                                                ...
                                                <div className="d-flex-column pb-0 pb-lg-5">
                                                  <p className="text-center"
                                                    style={{ color: product?.back_order_quantity == 0 || product && product.stock_status == "Out Stock" ? 'red' : '', fontSize: "10px" }}
                                                  >
                                                    {product?.back_order_quantity == 0 || product && product.stock_status == "Out Stock" ? <>&nbsp;</> : <>&nbsp;</>}
                                                  </p>
                                                  <button
                                  ...
                                  {products?.length > 5 && index === 5 && <div className="mb-3">
                                    <img
                                      className="w-100 d-block img-fluid mx-auto"
                                      itemID={2}
                                      // src={skinbanner}
                  ...
                  <div className="foot-section order-3">
                    ...
                    <div className="dynamic-content" dangerouslySetInnerHTML={{ __html: catadata?.description }}></div>
                  </div>
                </div>
