...
const HomePopup = ({ show, onHide, popupData }) => {
    ...
    useEffect(() => {
        ...
        if (popupData.length > 0) {
            ...
            const countDownData = popupData && popupData[0] && popupData[0].count_down;
    ...
    const handleCopyCode = () => {
        setShowButton(true)
        navigator.clipboard.writeText(popupData && popupData[0] && popupData[0].coupon_code);
    };
    ...
    return (
        <Modal id="HomePopup" show={modalPopupShow} onHide={handleClosePopup} size="lg">
            <Modal.Header closeButton>
            </Modal.Header>

            <div style={{ textAlign: "center", marginBottom: "10px" }} >
                <img width="800" height="400" src={popupData && popupData[0] && popupData[0].image} alt="Coupon Image" className="img-fluid popupimg" />
            </div>

            {/* <Modal.Footer> */}
            {popupData && popupData[0] && popupData[0].image &&
                <>
                    {popupData && popupData[0] && popupData[0].link && popupData[0].link != "undefined" &&
                    ...
                    {popupData && popupData[0] && popupData[0].coupon_code && popupData[0].coupon_code != "undefined" &&
                        <div className="text-center mx-auto d-block">
                            {showButton ? (
                                <div >
                                    <Button className="btn orderpopup-btn">
                                        Copied
                                    </Button>
                                </div>
                            ) : (
                                <div>
                                    ...
                                    <Button className="btn orderpopup-btn" onClick={handleCopyCode}>
