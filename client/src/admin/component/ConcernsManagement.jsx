...
import EditConcern from "./EditPolicy";

const ConcernManagement = () => {
  const [concernsList, setConcernList] = useState();
  const [createConcern, setCreateConcern] = useState(false);
  const [editForm, setEditForm] = useState(false);
  const [msg, setMsg] = useState("");
  const [show, setShow] = useState(false);

  const handleDataFromChild = (data) => {
    setCreateConcern(data);
  };
  ...
  const deleteConcern = async (concernId) => {
    ...
    try {
      ...
      const response = await api.delete(`/concern/${concernId}`);
      console.log(response, "del");
      if (response.status === 200) {
        setShow(true);
        setMsg(response.data.message);
        setConcernList(concernsList.filter((concern) => concern.id !== concernId));
      } else {
        setShow(false);
        setMsg(response.data.message);
      }
  ...
  const editConcern = async (concernId) => {
    setEditForm(true);
    setCreateConcern(false);
    try {
      // Perform the delete request
      await api.put(`/concern/${concernId}`); // Replace with your actual API endpoint
      // Update the state to reflect the deleted concern
      setConcernList(concernsList.filter((concern) => concern.id !== concernId));
    } catch (error) {
      console.error("Error deleting concern:", error);
    }
      ...
      <div className="inner_page profile_page">
        <div className="full_container">
          <div className="inner_container">
            <nav id="sidebar">
              <div className="sidebar_concern_1">
              ...
              <div className="sidebar_concern_2">
                <h4>General</h4>
            ...
            <div id="content">
              ...
              <div className="mh-100">
                <Navbar bg="light" expand="lg">
                  <Container>
                    <Navbar.Brand href="#home">Create Concern</Navbar.Brand>
                ...
                <Container fluid>
                  <Row>
                    ...
                    >
                      ...
                      {createConcern ? (
                        <ConcernPostForm sendDataToParent={handleDataFromChild} />
                      ) : (
                        <div>
                          <button
                            className="btn main_bt btn-sm float-right mb-2"
                            onClick={() => setCreateConcern(true)}
                          >
                            Create New
                          </button>
                          {editForm ? (
                                    ...
                                    <tr key={concern.id}>
                                      ...
                                      <td>
                                        <button
                                          ...
                                          onClick={() => editConcern(concern.id)}
                                        >
