...
const EditPolicy = () => {
  ...
  const [policy_date, setPolicy_date] = useState("");
  ...
  const handleInputChange = (e) => {
    ...
    setPolicy_date(new Date().toLocaleString());
    setCategory_id(Math.round(Math.random() * 1000));
    setPolicy_id(unique_id.slice(0, 8));
    setUrl("sample_policy_url");
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await api.post("/policys/1", data);

      if (response.status === 201) {
        // Policy post created successfully
        setPostStatus("Policy post created successfully.");
      } else {
        // Handle error response
        setPostStatus("Error: Unable to create policy post.");
      }
    } catch (error) {
