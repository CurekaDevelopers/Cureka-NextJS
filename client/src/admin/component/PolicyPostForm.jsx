...
const PolicyPostForm = () => {
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "policyName") setPolicyName(value);
    else if (name === "policyDetails") setPolicyDetails(value);
  };
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await api.post("/create-policy", data);

      if (response.status === 201) {
        // Policy created successfully
        setCreationStatus("Policy created successfully.");
      } else {
        // Handle error response
        setCreationStatus("Error: Unable to create policy.");
      }
    } catch (error) {
              ...
              <form onSubmit={handleFormSubmit}>
                <fieldset>
                  <div className="fields">
                    <label className="label_fields">Policy Name: </label>
                    <input
                      type="text"
                      name="policyName"
                      value={policyName}
                  ...
                  <br />
                  <div className="fields">
                    <label className="label_fields">Policy Details: </label>
                    <input
                      name="policyDetails"
                      value={policyDetails}
                      onChange={handleInputChange}
